    ...
    if request.method == 'POST':
        ...
        if form_past.is_valid():
            ...
            try:
                ...
                log.info('Output Values %s %s %s %s %s %s %s %s %s %s %s %s' % (region, number, ami, instance_type, name,
                         roles, runlist, vpc_selection, vpc, subnet, security_group, ssh_username))
            except:
                pass
...
def seelog(request):
    return render(request, 'aws_log_interactive.txt','')

def logpage(request):
    return render(request, 'aws_log.html', '')
