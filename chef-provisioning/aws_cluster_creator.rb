...
# Delete log
#File.delete("../templates/aws_log_interactive.txt")

# Executes the recently created Chef provisioning script and actually provision infrastructure.
# exec('chef-client -z aws_cluster_provisioning.rb > interactive.txt')
pid = Process.spawn('chef-client','-z', 'aws_cluster_provisioning.rb', :out => '../templates/aws_log_interactive.txt')
Process.detach(pid)

# Launch websocketd
pid = Process.spawn('../websocketd', '--address=127.0.0.1', '--port=8081', 'tail', '-f', '../templates/aws_log_interactive.txt')
Process.wait(pid)
exit
