...
OptionParser.new do |opt|
  ...
  opt.on('-r', '--region REGION') { |o| options[:region] = o }
  opt.on('-n', '--name NAME') { |o| options[:name] = o }
  opt.on('-N', '--number NUMBER') { |o| options[:number] = o }
  opt.on('-u', '--user USER') { |o| options[:ssh_username] = o }
  opt.on('-a', '--ami AMI-ID') { |o| options[:ami] = o }
  opt.on('-t', '--type INSTANCE-TYPE') { |o| options[:instance_type] = o }
  opt.on('-c', '--chef_url CHEF-URL') { |o| options[:chef_url] = o }
end.parse!
